        ...
        <div className='grid items-center gap-16 lg:grid-cols-[minmax(0,1fr)_460px]'>
          <div>
            ...
            <p className='mt-5 max-w-lg text-[17px] leading-[1.47] text-[var(--text-secondary)]'>
              ProofKit turns a project ZIP into a hackathon readiness report
              &mdash; runnable evidence, missing-risk items, README
              improvements, and judge testing steps.
            </p>
              ...
              <div className='space-y-3 p-5'>
                <div className='rounded-xl bg-[var(--bg-secondary)] p-4'>
                  <div className='flex items-end gap-2'>
                    ...
                    <span className='pb-1 text-[15px] text-[var(--text-tertiary)]'>
                      /100
                    </span>
                  </div>
                  <p className='mt-3 text-[13px] leading-[1.46] text-[var(--text-secondary)]'>
                    Local scan found setup, README, and tests &mdash; but repo
                    URL, demo video, license, and Codex usage still need
                    attention.
                  </p>
                </div>
                <div className='space-y-1.5'>
                  {readinessChecks.map((check) => (
                    <div
                      key={check.label}
                      className='flex items-center justify-between rounded-lg bg-[var(--bg-secondary)] px-3.5 py-2.5'
                    >
                      <span className='text-[13px] text-[var(--text-secondary)]'>
                        {check.label}
                      </span>
                      <span
                        className={`rounded-md px-2 py-0.5 text-[11px] font-semibold uppercase ${check.tone === 'pass' ? 'bg-[var(--color-pass-bg)] text-[#1a7f37]' : 'bg-[var(--color-fail-bg)] text-[#cf222e]'}`}
                      >
                        {check.status}
                      </span>
                    </div>
          ...
          <div className='mt-10 grid gap-3 sm:grid-cols-5'>
            {workflowSteps.map((step) => (
              ...
              >
                ...
                <p className='mt-3 text-[14px] font-medium leading-[1.43] text-[var(--text-primary)]'>
                  {step.text}
                </p>
              </div>
            ))}
        ...
        <div className='mx-auto max-w-6xl px-6'>
          <div className='grid gap-4 md:grid-cols-3'>
            {featureCards.map((feature) => (
              <article
                key={feature.title}
                className='rounded-2xl bg-white p-7 shadow-[0_0_0_1px_rgba(0,0,0,0.04),0_1px_2px_rgba(0,0,0,0.04)]'
              >
                <p className='text-[12px] font-semibold tracking-wide text-[var(--accent)] uppercase'>
                  {feature.eyebrow}
                </p>
                <h3 className='mt-3 text-[19px] font-semibold leading-[1.26] tracking-[-0.01em]'>
                  {feature.title}
                </h3>
                <p className='mt-3 text-[14px] leading-[1.57] text-[var(--text-secondary)]'>
                  {feature.body}
                </p>
              </article>
            ))}
      ...
      <section className='border-t border-[rgba(0,0,0,0.06)] bg-white py-20'>
        <div className='mx-auto max-w-lg px-6 text-center'>
          ...
          <p className='mt-3 text-[15px] text-[var(--text-secondary)]'>
            Upload a ZIP, configure your hackathon requirements, and get a
            judge-facing readiness report in seconds.
          </p>
          <Link
            href='/dashboard'
